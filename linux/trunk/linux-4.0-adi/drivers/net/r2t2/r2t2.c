...
static struct xilinx_dma_chan *getChan(enum dma_transfer_direction dir) {
	...
	for (i=0;i<XILINX_DMA_MAX_CHANS_PER_DEVICE;i++)
		if (xdev && xdev->chan[i] && xdev->chan[i]->direction == dir)
			return xdev->chan[i];
	return NULL;
}
...
static int rad_prepare_rx(void) {
	...
	if (!chan) {
		printk(KERN_ERR "r2t2: dma channel not found");
		return 0;
	}
	desc = chan->desc;
...
static int rad_xmit(struct sk_buff *skb, struct net_device *net_dev) {
	...
	if ((desc->hw.status & XILINX_DMA_SR_CMP_MASK) == 0) {
		pr_debug("r2t2: tx dma busy, packet ignored");
	...
	if (skb->len > R2T2_DMA_BUF_SIZE - HEADER_LEN) {
		printk(KERN_ERR "r2t2: packet to long, ignored");
		goto error;
	}
				 ...
				 struct device_node *node, u32 feature)
{
	...
	for (i=0;i<DESC_CNT;i++) {
			...
			new->buf = dma_alloc_coherent(chan->dev, R2T2_DMA_BUF_SIZE, &new->dma_buf, GFP_KERNEL); 
			if (!new->buf) {
				printk(KERN_ERR "r2t2: error dma_alloc");
				goto error;
			}

			new->hw.buf_addr = new->dma_buf;
			new->hw.control = R2T2_DMA_BUF_SIZE | XILINX_DMA_BD_SOP | XILINX_DMA_BD_EOP; 
			new->hw.status = XILINX_DMA_SR_CMP_MASK;

			memset(new->buf, 0, R2T2_DMA_BUF_SIZE);
	}
...
static int r2t2_dma_probe(struct platform_device *pdev)
{
	...
	//rad_prepare_rx();
	dev_info(&pdev->dev, "Probing xilinx r2t2 dma engine...Successful\n");
...
static int r2t2_dma_remove(struct platform_device *pdev)
{
...
static const struct of_device_id r2t2_dma_of_match[] = {
	{ .compatible = "xlnx,axi-dma-1.00.a", },
	{}
};
MODULE_DEVICE_TABLE(of, r2t2_dma_of_match);

static struct platform_driver r2t2_dma_driver = {
	.driver = {
		.name = "xilinx-dma",
		.of_match_table = r2t2_dma_of_match,
	},
	.probe = r2t2_dma_probe,
	.remove = r2t2_dma_remove,
};
...
static int __init rad_init(void) {
	...
	err =  platform_driver_register(&r2t2_dma_driver);
	if (err) {
		printk(KERN_ERR "Error register r2t2 dma platform driver\n");
		return err;
	}
...
static void __exit rad_clean(void)
{
	unregister_netdev(netdev_rad);
	free_netdev(netdev_rad);
	platform_driver_unregister(&r2t2_dma_driver);
}
