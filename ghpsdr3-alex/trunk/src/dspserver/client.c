...
static const int extSampleRate = 192000;
static int timing=0;
    ...
    while (evbuffer_get_length(inbuf) >= MSG_SIZE) {
        ...
        if(strncmp(cmd,"q",1)==0){	
            ...
            */
            ...
            if (tokenize_cmd(&saveptr, tokens, 2) != 2)
                goto badcommand;
            sdr_log(SDR_LOG_INFO,"Spectrum fps set to = '%s', size to %s\n",tokens[1],tokens[0]);
            sem_wait(&bufferevent_semaphore);
            if (slave) {
...
//
	...
	if (config.externalFFT) {
		client_samples[9] =(extSampleRate>>24)&0xFF; // sample rate
		client_samples[10]=(extSampleRate>>16)&0xFF;
		client_samples[11]=(extSampleRate>>8) &0xFF;
		client_samples[12]= extSampleRate     &0xFF;
	} else {
