...
double LO_offset = 0;  // LO offset 0khz
...
static pthread_t fft_thread_id;
...
extern float spectrumBuffer[SAMPLE_BUFFER_SIZE];
...
float fft_buffer[SAMPLE_BUFFER_SIZE];
...
void* iq_thread(void* arg) {
	...
	while(1) {
		int bytes_read;
		int offset = 0;
		unsigned long long rx_sequence = 0;
#ifdef SMALL_PACKETS
		while(1) {
			...
			if(buffer.offset==0) {
				...
				if((rx_sequence==buffer.sequence) && (offset==buffer.offset)) {
					...
					fprintf(stderr,"missing IQ frames %llu %llu - %ul %ul\n", rx_sequence, buffer.sequence, offset, buffer.offset);
				}
			} // if(buffer.offset==0)
		} // end while(1)
#else
...
void* fft_thread(void* arg) {
	int fft_socket;
	struct sockaddr_in fft_addr;
	int fft_length = sizeof(fft_addr);
	BUFFER buffer;
	int on=1;
	const char *dspserver_address = (const char *)arg;

	sdr_thread_register("fft_thread");
	fprintf(stderr,"fft_thread\n");

	// create a socket to receive fft from the server
	fft_socket=socket(PF_INET,SOCK_DGRAM,IPPROTO_UDP);
	if(fft_socket<0) {
		perror("fft_thread: create fft socket failed");
		exit(1);
	}

	setsockopt(fft_socket, SOL_SOCKET, SO_REUSEADDR, &on, sizeof(on));

	memset(&fft_addr,0,fft_length);

	fft_addr.sin_family=AF_INET;
	fprintf(stderr,"fft_thread: fft address: %s\n", dspserver_address);
	fft_addr.sin_addr.s_addr=inet_addr(dspserver_address);
	fft_addr.sin_port=htons(FFT_PORT+(receiver*2));

	if(bind(fft_socket,(struct sockaddr*)&fft_addr,fft_length)<0) {
		perror("fft_thread: bind socket failed for fft socket");
		exit(1);
	}

	fprintf(stderr,"fft_thread: fft bound to port %d socket=%d\n",htons(fft_addr.sin_port),fft_socket);
	...
	while(1) {
		...
		while(1) {
			bytes_read=recvfrom(fft_socket,(char*)&buffer,sizeof(buffer),0,(struct sockaddr*)&fft_addr,(socklen_t *)&fft_length);
			if(bytes_read<0) {
				perror("recvfrom socket failed for fft buffer");
				exit(1);
			}
			...
			if(buffer.offset==0) {
				if (offset/2*4 >= sizeof(spectrumBuffer)) {
					printf ("spectrum buffer overflow\n");
				} else {
					//printf ("%f %f\n",fft_buffer[0],log10((fft_buffer[0])*fft_buffer[0]+fft_buffer[1]*fft_buffer[1])*10);
					for (int i=0;i<offset/2;i++) {
#define FFT_OFFSET 	30
						double ampl = FFT_OFFSET+log10((fft_buffer[i*2])*fft_buffer[i*2]+fft_buffer[i*2+1]*fft_buffer[i*2+1])*10;
						for (int j=0;j<4;j++) {
							if (i<offset/4)
								spectrumBuffer[offset+i*4+j] = ampl;
							else
								spectrumBuffer[(i*4+j)-offset] = ampl;
						}
					}
				}
				offset = 0;
			...
			if((rx_sequence==buffer.sequence) && (offset*sizeof(fft_buffer[0]) == buffer.offset)) {
				if(offset + buffer.length >= sizeof(fft_buffer)) {
					printf ("fft buffer overflow\n");
					offset=0;
					break;
				}
				memcpy((char *)&fft_buffer[offset],(char *)&buffer.data[0], buffer.length);
				offset+=buffer.length/sizeof(fft_buffer[0]);
			} else {
				fprintf(stderr,"missing FFT frames %llu %llu - %ul %ul\n", 
						rx_sequence, buffer.sequence, offset*sizeof(fft_buffer[0]), buffer.offset);
			}
			rx_sequence++;
		}
	}
...
int make_connection(int externalFFT) {
	char *token, *saveptr;
	...
	send_command(command, response);
	if (externalFFT) {
		sprintf(command,"start fft %d",FFT_PORT+(receiver*2));
		send_command(command, response);
	}
 ...
 * @return 
 */
	int ozy_init(const char *server_address, const char *dspserver_address, int externalFFT) {
		int rc;
		struct hostent *h;
		int on=1;
		...
		if(make_connection(externalFFT)) {
		...
		if (externalFFT) {
			// create a thread to listen for fft frames
			rc=pthread_create(&fft_thread_id,NULL,fft_thread,(void*)dspserver_address);
			if(rc != 0) {
				fprintf(stderr,"pthread_create failed on fft_thread: rc=%d\n", rc);
			}
		}
