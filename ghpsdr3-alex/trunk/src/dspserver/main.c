...
struct option longOptions[] = {
    ...
    {"extfft",no_argument, NULL, OPT_EXT_FFT},
#ifdef THREAD_DEBUG
...
void processCommands(int argc,char** argv,struct dspserver_config *config) {
    ...
    while((c=getopt_long(argc,argv,shortOptions,longOptions,NULL))!=-1) {
       ...
       default:
                ...
                fprintf(stderr,"            --extfft (R2T2: use fft from fpga)\n");
...
int main(int argc,char* argv[]) {
    ...
    G711A_init();
	printf ("########### extfft %i\n",config.externalFFT);
    ozy_init(config.server_address, config.dspserver_address, config.externalFFT);   // create and starts iq_thread in ozy.c in order to
