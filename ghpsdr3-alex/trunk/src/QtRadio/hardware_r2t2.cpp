#include <QtGui>
#include "hardware.h"
#include "hardware_r2t2.h"
#include "UI.h"

HardwareR2t2 :: HardwareR2t2 (Connection *pC, QWidget *pW): DlgHardware (pC, pW)
{
    // Attenuator
...
void HardwareR2t2 :: attClicked(int newVal)
{
...
void HardwareR2t2 :: antClicked(int n)
{
   qDebug() << "Antenna: " << n ;
   if (antennaVal != n) {
      QString command;
...
void HardwareR2t2 :: preselClicked(int n)
{
...
void HardwareR2t2 :: preampChanged(int state)
{
...
void HardwareR2t2 :: processAnswer (QStringList list)
{
    if (list[0] == "*getserial?") {
...
HardwareR2t2 :: ~HardwareR2t2 ()
{

}

namespace {
   struct RegisterHw<HardwareR2t2> r("R2T2");
}
