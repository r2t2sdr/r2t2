...
void setRxAtt(unsigned int att, int rx) {
	if (att>31) 
		att=31;
	att = att << 1;
	if (rx==0)
...
void initADC() {
	...
	for (i=0;i<sizeof(adc_init_data)/sizeof(adc_init_data[0]/2);i++) {
		...
		// printf("write reg :  %04x %02x\n", adc_init_data[i*2],adc_init_data[i*2+1]);
		if (i==0)
			usleep(100000);
		if (ret) {
...
#if 0
	printf("read ADC %02x\n",readReg(adc, ADC_SPI_PORT));
	printf("read ADC %02x\n",readReg(adc, ADC_CHIP_ID));
	printf("read ADC %02x\n",readReg(adc, ADC_CHIP_GRADE));
	printf("read ADC %02x\n",readReg(adc, ADC_DEVICE_INDEX));
	printf("read ADC %02x\n",readReg(adc, 0x21));
	printf("read ADC %02x\n",readReg(adc, 0x100));
#endif
}
...
void initDAC() {
	...
	for (i=0;i<sizeof(dac_init_data)/sizeof(dac_init_data[0])/2;i++) {
		...
		// printf("write reg :  %04x %02x\n", dac_init_data[i*2],dac_init_data[i*2+1]);
		if (i==0)
			usleep(100000);
		if (ret) {
