...
AMDemod::AMDemod(std::string name) : ProcessBlock(name, 1, 1) {
...
int AMDemod::process(cfloat_t* in, int cnt) {
	int processed = 0;
	float *out = (float*)in;
	while (processed < cnt) {
		cfloat_t x = *in++;
		*out++ = sqrt(x.re*x.re + x.im*x.im);
		processed++;
	}
	return processed;
}

int AMDemod::receive(std::shared_ptr<ProcessBuffer> buf, uint32_t input, int recursion) {
